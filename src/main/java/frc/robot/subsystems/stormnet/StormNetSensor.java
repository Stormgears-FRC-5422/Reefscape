...
public class StormNetSensor {
	...
	public int getTimer() {
		int[] receiveBuffer = new int[3];
		boolean b = fetchInts(":", "Timer", receiveBuffer);
		return b ? receiveBuffer[0] : -1;
	}
...
// low-level i/o routines	
	...
	protected boolean fetchShorts(String command, String commandName, short[] shortArray) {
		...
		if (result) parseShorts(receiveBuffer, shortArray);
		return result;
	}

	protected void parseShorts(byte[] receiveBuffer, short[] shortArray) {
	...
	protected boolean fetchInts(String command, String commandName, int[] intArray) {
		...
		if (result) parseInts(receiveBuffer, intArray);
		return result;
	}

	protected void parseInts(byte[] receiveBuffer, int[] intArray) {
		ByteBuffer buffer = ByteBuffer.wrap(receiveBuffer);
		buffer.order(ByteOrder.LITTLE_ENDIAN);
	...
	protected boolean fetchLongs(String command, String commandName, long[] longArray) {
		...
		if (result) parseLongs(receiveBuffer, longArray);
		return result;
	}

	protected void parseLongs(byte[] receiveBuffer, long[] longArray) {
		ByteBuffer buffer = ByteBuffer.wrap(receiveBuffer);
		buffer.order(ByteOrder.LITTLE_ENDIAN);
	...
	protected boolean fetchFloats(String command, String commandName, float[] floatArray) {
		...
		if (result) parseFloats(receiveBuffer, floatArray);
		return result;
	}

	protected void parseFloats(byte[] receiveBuffer, float[] floatArray) {
		ByteBuffer buffer = ByteBuffer.wrap(receiveBuffer);
		buffer.order(ByteOrder.LITTLE_ENDIAN);
	...
	protected boolean fetchDoubles(String command, String commandName, double[] doubleArray) {
		...
		if (result) parseDoubles(receiveBuffer, doubleArray);
		return result;
	}

	protected void parseDoubles(byte[] receiveBuffer, double[] doubleArray) {
		ByteBuffer buffer = ByteBuffer.wrap(receiveBuffer);
		buffer.order(ByteOrder.LITTLE_ENDIAN);
	...
	// Just echo PING - should return the id of the sensor - 1 byte
	public byte ping() {
